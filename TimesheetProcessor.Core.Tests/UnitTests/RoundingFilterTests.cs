...
namespace TimesheetProcessor.Core.Tests.UnitTests
{
    public class RoundingFilterTests
    {
        ...
        [Fact]
        public void TestRoundToNearestQuarterHourFilter()
        {
            var timesheet = new Builder()
                .Day("2020-02-03").AddEntry("admin", "0:1:1").AddEntry("building, prod1", "0:14:30")
                .AndOnDay("2020-02-04").AddEntry("inventing", "0:16:1").AddEntry("building, prod2", "0:27:10")
                .AndOnDay("2020-02-05").AddEntry("admin", "0:0:1")
                .AndOnDay("2020-02-06").AddEntry("admin", "0:0:1")
                .AndOnDay("2020-02-07").AddEntry("admin", "0:0:1")
                .AndOnDay("2020-02-08").AddEntry("admin", "0:0:1")
            ...
            var filter = new RoundToNearestQuarterHourFilter();
            timesheet = filter.Filter(timesheet);

            Assert.Equal(new TimeSpan(0, 15, 0), timesheet.Days[0].Entries[0].TimeSpent);
            Assert.Equal(new TimeSpan(0, 30, 0), timesheet.Days[0].TotalTimeSpent);
            Assert.Equal(new TimeSpan(0, 45, 0), timesheet.Days[1].TotalTimeSpent);
            Assert.Equal(new TimeSpan(1, 30, 0), timesheet.Tags.First(x => x.TagId == "admin").TotalTimeSpent);

            Assert.Equal(new TimeSpan(2, 30, 0), timesheet.TotalTimeSpent);
        }
    }
}